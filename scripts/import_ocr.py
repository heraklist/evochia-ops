...
def main():
    p = argparse.ArgumentParser(description="OCR import stub (provider-agnostic deterministic interface)")
    p.add_argument("--input", required=True, help="OCR structured rows json (stub input)")
    ...
    # Layout gating v1: if layout mismatch => no partial parse
        ...
        for i, x in enumerate(rows_in, start=1):
            ...
            if missing:
                add_issue(
                    ...
                    "Critical field missing; row sent to needs_review",
                    offer_id=offer_id,
            ...
            if unit_norm is None:
                add_issue(
                    ...
                    "Unsupported OCR unit; row sent to needs_review",
                    offer_id=offer_id,
                    raw_unit=unit_src,
                )
                needs_review.append(
            ...
            if qty is None or qty <= 0 or price is None:
                add_issue(
                    issues,
                    "WARNING",
                    "IMPORT-NUMERIC-PARSE-FAILED",
                    "qty/net parse failed; row sent to needs_review",
                    offer_id=offer_id,
                )
                needs_review.append(
                    {
            ...
            rows.append(
                {
                    ...
                    "source": f"import_ocr:{Path(args.input).name}",
    ...
    batch = {
        ...
        "files": [{"path": str(args.input), "kind": "json"}],
        "stats": {
            "rows_in": len(rows_in),
            "rows_out": len(rows),
